...
Game::Game(int w, int h, const std::string& title) : width(w), height(h) {
    // Inside your Game class where window is created
    sf::ContextSettings settings;
    settings.majorVersion = 4;
    settings.minorVersion = 6;
    settings.attributeFlags = sf::ContextSettings::Attribute::Core; // Vital for modern GL
    settings.depthBits = 24;
    settings.stencilBits = 8;
    settings.antiAliasingLevel = 4;

    // Update the create call to include settings
    window.create(sf::VideoMode({width, height}), title, sf::Style::Default);
    //glWindow.create(sf::VideoMode({400, 400}), "OpenGL Renderer", sf::State::Windowed, settings);
    // 3. Initialize GLAD while the glWindow is active
...
void Game::update(float dt) {
    ...
    try {
        ...
        title.rotate(sf::degrees(1.0f * dt)); // Rotate 45 degrees per second
    } catch (...) {
